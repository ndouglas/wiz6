...
2b. **⚠ CONFLICT (settle via parity harness) — sex vs alignment record offset.** Phase 1 §1/§3 placed the screen-03 SEX choice at record **0x19e**. The pre-existing `CHARACTER_RECORD` struct labels **0x19e = alignment** (but with a "msg lookup +0x8c" — and 0x8c/0x8d ARE the MALE/FEMALE strings, which actually points back to sex) and **0x1a1 = sex** (portrait-table lookup `cs:0x526[sex*2]`). Stage-A A6 added the schema `sex` field at **0x1a1** following the existing struct. Resolve by decoding stock characters via `decodePcfile` (handles the pcfile header — naive byte slicing hits the wrong offsets) and correlating 0x19e/0x1a1 against known stock-character sexes. The Phase-2 sex picker must write whichever offset proves correct. Task A8's parity harness should validate this.
