...
- **Skill-train floor model.** **Confirmed by user:** floor = entry-time skill values snapshotted at screen entry. Points earned in prior phases (racial/class baseline at character creation, OR level-up gains, OR random gameplay improvements like sword-skill-up-on-hit) are locked — only the budget being currently distributed can be allocated and re-allocated. This same pattern generalizes to **future level-up screens**: the implementation of `skillFloors` here should be reusable when the level-up UI is ported.
