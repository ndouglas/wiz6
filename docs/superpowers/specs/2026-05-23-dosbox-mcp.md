...
### Phase 1 — `BssStruct` schemas in `@wiz6/data`
...
### Phase 3 — DOSBox-X bridge (the unknown)

The riskiest phase; do it early so we know if the whole approach is viable.

- Subagent or focused investigation: read DOSBox-X's debugger documentation + source.
- Build `debugger-console.ts`: launch DOSBox-X with debug mode, send commands, parse responses.
- Validate with smoke tests: launch → read 16 bytes from a known address → kill.

If the debugger-console approach proves unworkable (e.g. headless mode broken on macOS), fall back to save-state snapshot reads as the v1 backend — slower, but unblocks the rest of the phases.

### Phase 4 — MCP server scaffold + lifecycle tools
...
### Phase 9 — First-payoff experiment
