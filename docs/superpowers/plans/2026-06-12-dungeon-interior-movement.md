...
> **⛔ BLOCKED 2026-06-12 — the core hypothesis is FALSIFIED (live-verified). Do not execute Tasks 2–5 as written.**
> Task 1 (the `openAllDoors` poke helper) shipped and runs (commit 2fe1bca; pokes 12 type-7 doors), but the re-capture had **zero effect on reachability** — (124,120), the cell north of the canonical door, is still unreached (73 cells, ≈ the old 74). A focused `stepprobe` at the door cell (124,121,f2) proved why: the per-cell **north wall plane already reads OPEN (`liveN=0`)**, yet the engine **refuses to step** (`UP moved=false`), and poking the `+0x240` door-record word changes nothing.
> **Root cause:** the door-record `+0x240` wall-plane (pointer `0x4fa8`; what FORCE/PICK success edits via `FUN_0000_88af`, and what this plan pokes) is **NOT** what the movement pass-gate reads. The pass-gate `maze_can_pass_doors_and_walls` (0x309d) reads the **per-cell maze table at pointer `0x4faa`**: special-index `+0x1f8` (values 1/2 arm an early pass-block), door-orientation `+0x378`, wall planes `+0x60`/`+0x120` (refs: `docs/re/findings/maze-door-open-mechanic.json`, `maze-open-door-menu.json`). To open a door for the engine's OWN movement you must replicate `door_set` (`FUN_0000_6d62`) open semantics on the **cell** table — set `+0x1f8` special-index = 7 (engine-opened) + `+0x378` door-orientation (and likely clear `+0x60`/`+0x120`) at each door cell — NOT the `+0x240` record edit. Residual uncertainty: the cell-index x/y order (stepprobe uses `z*64 + x*8 + y`; the finding says `z*64 + y*8 + x`), the 4-bit/2-bit MSB-first packing, and whether the wall planes also need clearing.
> **Awaiting Nate's direction** on whether to pursue the revised cell-table poke, tighten the fallback model instead, or escalate to the deeper #087 collision RE. See the session escalation.
