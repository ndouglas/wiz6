#!/usr/bin/env python3
"""Apply the curated wpcvw.ovr function-naming pass.

Loads names from docs/re/findings/wpcvw-naming-pass.json and applies them to
...
def main() -> int:
    ...
    ap.add_argument("--binary", default="wpcvw.ovr")
    ap.add_argument(
        "--findings",
        default="docs/re/findings/wpcvw-naming-pass.json",
        help="path to the naming-pass JSON (default: docs/re/findings/wpcvw-naming-pass.json)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    tx_id = None
    if not args.dry_run:
        tx_id = program.startTransaction("apply_wpcvw_names")
    tx_committed = False
    try:
        for r in rename_list:
            ...
            # wpcvw.ovr is BinaryLoader (no MZ header); file_off == Ghidra addr.
            ...
            if f is None:
                ...
                if f is None or f.getEntryPoint().getOffset() != off:
                    ...
                    try:
                        ...
                        body_addr = addr
                        # Use createFunction(name, entry_point, body, source)
                        # If body is None Ghidra will compute it from the entry
                        new_fn = fm.createFunction(
                            new_name, body_addr, None, SourceType.USER_DEFINED
                        )
                        if new_fn is None:
