#!/usr/bin/env python3
"""Apply the curated wroot.exe function-naming pass.

Loads names from docs/re/findings/wroot-naming-pass.json and applies them to
...
Usage:
    python3 tools/ghidra/scripts/apply_wroot_names.py
    python3 tools/ghidra/scripts/apply_wroot_names.py --dry-run
...
def main() -> int:
    ...
    ap.add_argument(
        "--findings",
        default="docs/re/findings/wroot-naming-pass.json",
        help="path to the naming-pass JSON (default: docs/re/findings/wroot-naming-pass.json)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    tx_id = None
    if not args.dry_run:
        tx_id = program.startTransaction("apply_wroot_names")
    tx_committed = False
    try:
