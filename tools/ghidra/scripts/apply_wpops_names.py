#!/usr/bin/env python3
"""Apply the curated wpops.ovr function-naming pass.

Loads names from docs/re/findings/wpops-naming-pass.json and applies them to
...
Usage:
    python3 tools/ghidra/scripts/apply_wpops_names.py
    python3 tools/ghidra/scripts/apply_wpops_names.py --dry-run

NOTE: Do NOT run this with the Ghidra GUI open on the project — it holds an
exclusive lock. Close the GUI first.

Overlay-specific notes for wpops.ovr:
- BinaryLoader-loaded raw overlay (no MZ header), so file offset == Ghidra
  address. We do NOT add 0x10000.
- wpops uses the CANONICAL state-machine layout (unlike wmexe):
    * 14-byte header at 0x00..0x0d
    * 0x0e: dispatch entry (cmp [0x363a],0x0c; jnz +3; call 0x4586; ret)
    * 0x4586: state-0c handler (action selection + monster AI driver)
- Ghidra did NOT auto-detect 0x0e — this script creates it.

wpops = PRE-OPERATIONS / POPUP-PICKER = the combat-action selection layer.
It handles state 0x0c, between wmele 0x0b (round start) and wmexe 0x0d
(action execution). The state-0c handler at 0x4586 runs the party action
picker (FUN_3185), then the monster AI driver (FUN_44e1), then writes
*0x363a = 0x0d to transition to wmexe.
"""
...
def main() -> int:
    ...
    ap.add_argument("--binary", default="wpops.ovr")
    ap.add_argument(
        "--findings",
        default="docs/re/findings/wpops-naming-pass.json",
        help="path to the naming-pass JSON (default: docs/re/findings/wpops-naming-pass.json)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    tx_id = None
    if not args.dry_run:
        tx_id = program.startTransaction("apply_wpops_names")
    tx_committed = False
    try:
        for r in rename_list:
            ...
            # wpops.ovr is BinaryLoader (no MZ header); file_off == Ghidra addr.
            ...
            if f is None:
                ...
                if f is None or f.getEntryPoint().getOffset() != off:
                    ...
                    # For wpops, 0x000e (dispatch entry) is NOT auto-detected.
                    if args.dry_run:
