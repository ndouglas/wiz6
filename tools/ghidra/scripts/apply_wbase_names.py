#!/usr/bin/env python3
"""Apply the curated wbase.ovr function-naming pass.

Loads names from docs/re/findings/wbase-main-menu.json and applies them to
the Ghidra project. Each function gets:
  - name set to `new` field
  - SourceType.USER_DEFINED so the rename survives re-opens

Additionally creates 12 functions at addresses that are reached only via the
overlay's runtime dispatch (no direct CALL/JMP for auto-analysis to follow):
  - 0x0c (wbase_overlay_entry — entry dispatch at 0x0e is mid-function)
  - 0x2cf7..0x2dad (the 9 menu-option handlers reached via the inline jump
    table at 0x2dbb / runtime addr 0x731f)
  - 0x2a70 (wbase_unload_all_party_members, reached only by tail-calls from
    option handlers 5/7/8)
  - 0x253a (wbase_add_party_member_action, reached only from option-0 handler)

Re-runnable: rewriting the JSON and re-running this script will reconverge
the project's annotations to whatever the JSON says. Functions already at
the target name are no-ops.

Usage:
    python3 tools/ghidra/scripts/apply_wbase_names.py
    python3 tools/ghidra/scripts/apply_wbase_names.py --dry-run

NOTE: Do NOT run this with the Ghidra GUI open on the project — it holds an
exclusive lock. Close the GUI first.

Overlay-specific: wbase.ovr is a BinaryLoader-loaded raw overlay (no MZ
...
def main() -> int:
    ...
    ap.add_argument("--binary", default="wbase.ovr")
    ap.add_argument(
        "--findings",
        default="docs/re/findings/wbase-main-menu.json",
        help="path to the naming-pass JSON (default: docs/re/findings/wbase-main-menu.json)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    tx_id = None
    if not args.dry_run:
        tx_id = program.startTransaction("apply_wbase_names")
    tx_committed = False
    try:
        ...
        for r in create_list:
            ...
            # wbase.ovr is BinaryLoader (no MZ header); file_off == Ghidra addr.
