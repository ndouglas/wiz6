#!/usr/bin/env python3
"""Apply the curated wtrea.ovr function-naming pass.

Loads names from docs/re/findings/wtrea-naming-pass.json and applies them to
...
Usage:
    python3 tools/ghidra/scripts/apply_wtrea_names.py
    python3 tools/ghidra/scripts/apply_wtrea_names.py --dry-run

NOTE: Do NOT run this with the Ghidra GUI open on the project — it holds an
exclusive lock. Close the GUI first.

Overlay-specific: wtrea.ovr is a BinaryLoader-loaded raw overlay (no MZ
header), so file offset == Ghidra address. We do NOT add 0x10000 to the
address like the wroot.exe script does.

Special case: wtrea's dispatch entry at file 0x0e is not automatically
detected by Ghidra. This script creates a function there before renaming if
the JSON includes an entry at 0x0e. Unlike wpcmk/wmnpc, wtrea.ovr IS a true
state-machine handler: the dispatch entry at 0x0e reads `*0x363a` and
dispatches to state-handler 0x561c (state 0xf, post-combat treasure) or
0x56c5 (state 0x15, in-dungeon chest), both writing `*0x363a=0x16` on exit.
"""
...
def main() -> int:
    ...
    ap.add_argument("--binary", default="wtrea.ovr")
    ap.add_argument(
        "--findings",
        default="docs/re/findings/wtrea-naming-pass.json",
        help="path to the naming-pass JSON (default: docs/re/findings/wtrea-naming-pass.json)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    tx_id = None
    if not args.dry_run:
        tx_id = program.startTransaction("apply_wtrea_names")
    tx_committed = False
    try:
        for r in rename_list:
            ...
            # wtrea.ovr is BinaryLoader (no MZ header); file_off == Ghidra addr.
