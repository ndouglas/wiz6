#!/usr/bin/env python3
"""Apply the curated wmaze.ovr function-naming pass.

Loads names from docs/re/findings/wmaze-naming-pass.json and applies them to
...
Usage:
    python3 tools/ghidra/scripts/apply_wmaze_names.py
    python3 tools/ghidra/scripts/apply_wmaze_names.py --dry-run

NOTE: Do NOT run this with the Ghidra GUI open on the project — it holds an
exclusive lock. Close the GUI first.

Overlay-specific: wmaze.ovr is a BinaryLoader-loaded raw overlay (no MZ
...
def main() -> int:
    ...
    ap.add_argument("--binary", default="wmaze.ovr")
    ap.add_argument(
        "--findings",
        default="docs/re/findings/wmaze-naming-pass.json",
        help="path to the naming-pass JSON (default: docs/re/findings/wmaze-naming-pass.json)",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    tx_id = None
    if not args.dry_run:
        tx_id = program.startTransaction("apply_wmaze_names")
    tx_committed = False
    try:
        for r in rename_list:
            ...
            # wmaze.ovr is BinaryLoader (no MZ header); file_off == Ghidra addr.
