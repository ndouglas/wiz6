...
function main(): void {
  ...
  const payload = JSON.stringify({
    _comment: 'CAPTURE-REPLAY viewport oracles for faithful level-0 (one engine viewport per reachable (gx,gy,facing) position; keyed by posKey). viewportB64 = base64(gzip(176x112 EGA-index)). Built by tools/parity/build-viewport-oracles.ts from collmap/collcapture framebuffers.',
