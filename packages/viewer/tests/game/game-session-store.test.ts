...
describe('GameSession store', () => {
  ...
  it('init WITH scriptedEntry seeds party = start coords', () => {
    initGameSession(LEVEL_WITH_ENTRY);
    const s = readGameSession();
    expect(s?.party).toEqual({ gx: 127, gy: 118, z: 0, facing: 0 });
  });

  it('init WITH scriptedEntry seeds entryMode:narration', () => {
    initGameSession(LEVEL_WITH_ENTRY);
    const s = readGameSession();
    expect(s?.entryMode).toBe('narration');
  });

  it('init WITH scriptedEntry seeds stepsRemaining = steps', () => {
    initGameSession(LEVEL_WITH_ENTRY);
    const s = readGameSession();
  ...
  it('entrance is NOT overwritten by scriptedEntry.start', () => {
    initGameSession(LEVEL_WITH_ENTRY);
    const s = readGameSession();
    // level.entrance stays gy=121; scriptedEntry.start is gy=118 (party only)
  ...
  it('readGameSession returns null for a stored v1 blob', () => {
    // Write a raw schemaVersion:1 object into localStorage (the old schema).
