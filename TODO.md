...
- #032 [open] — Port WPCVW SKILL action (RE done 2026-06-01; **active port target** — read-only viewer)
  ...
  - **Stage 1 DONE (2026-06-01):** binary-anchored skill-name correction — `SKILL_SLOT_NAMES` replaced with the engine map (msg 5500+slot; `docs/re/findings/wpcvw-skill-names.json`), weaponry 0-9 reordered + slots 10/17-21 are real skills (SWIMMING/DEFENSE/SPEED/MOVEMENT/AIM/POWER); `pcfile.ts` index-map comment fixed. New `@wiz6/data` `character-view/skill-viewer.ts` (`SKILL_CATEGORIES`, `skillRowVisible`, `skillViewerRows`; visible iff class-can-train OR level>0). 578 data tests green. Engineering Notes card "The Skill Names Hiding In Plain Sight" added. No shipped UI changed (skill-train already names via `skillName(db,slot)`). **Next: Stage 2 (composer) → 3 (reducer/page wiring; surface +0x1a8 skill-points) → 4 (engine fixture + pixel-parity) → 5 (e2e) → 6 (verify MEDIUM bits).**
