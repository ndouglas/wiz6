...
- #076 [open] — Port the maze 3D view (renderer LOCATED: it's in ega.drv, not wmaze.ovr)
  ...
  - **Next:** decompile ega.drv 0x1fd0 (Ghidra) for the full per-plane loop; find the WALL caller that copies the template + sets args (si/di/bx/dx/cl, source texture ptr, page flag) per depth-slice; map si/cl/screen-col to the convergence/seam tables for U/V; decode one wall texture end-to-end (capture the off-screen +0x2000 page mid-frame) → pixel-match → port the EGA-planar RMW rasterizer to TS. Framebuffer-oracle remains a fallback.
