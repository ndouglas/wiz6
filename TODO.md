...
- #073 [open] — Port camp WPCVW SPELL — read-only spellbook viewer (RE done; build is a fresh-session effort)
  ...
  - Build = (ASSAY-sized): reducer sub-states (school-grid → spell-sublist, read-only) + a spellbook composer (reuse creation picker) + CharacterViewPage wiring + pixel-parity + e2e.
  - **PREREQ (a) DONE 2026-06-03:** known-spell enumeration RE'd + verified + data model shipped. Learned spells = a flat per-spell-id BITSET at record +0x188 (`Character.spellSlotsKnown`); `@wiz6/data` `character-view/known-spells.ts` (`knownSpells`/`knownSpellsBySchool`/`isSpellKnown`, cost = spell-table b2). The forward bridge now carries spellSlotsKnown. RE: `docs/re/findings/wpcvw-known-spells.json` (verified vs pinned roster: TREON {0,48}, PENTAG {9,37}, NOBAL {50,64}). Commit 30268d9.
  - **PREREQ (b) (caster fixture) — REMAINING but easy:** the PINNED roster already has casters (TREON/PENTAG = Mage slots 4/5, NOBAL = Priest slot 3) — no new roster needed. Add a `review-<caster>-spellbook` recipe (boot pinned, form a party incl. a caster, REVIEW → SPELL → school grid + a school's sublist) + mint the fixture, mirroring `review-twink-shuriken`. The `build-state.ts` pcfileFixture-replay fix + the recipe pattern are in place.
  - **REMAINING (the UI build):** reducer `spell-grid`/`spell-sublist` sub-states (read-only; arrows nav, ENTER drills in, ESC backs out) wired off the SPELL action (currently `return state // SPELL handler is SP3` in character-view-reducer.ts) + a spellbook composer (adapt `roster/creation/.../compose-spell-panel` + the school grid) fed by `knownSpellsBySchool(member)` + CharacterViewPage wiring + pixel-parity fixture(s) + e2e.
