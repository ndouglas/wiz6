...
- #076 [open] — Port the maze 3D view (renderer LOCATED: it's in ega.drv, not wmaze.ovr)
  ...
  - **Next:** read the live wall-piece descriptor table (0x18-stride: src ptr/mask/w/h) + tile-index table @0x17a + LUT @0x192; resolve the wmaze far-call site (wmaze+0x681a/+0x6aa1 → confirm it far-calls ega entry 10 off 0x2b) and capture the per-slot arg block (piece string, clip, flags) for the reference frame; decode one wall texture end-to-end (descriptor → 4-plane source → FUN_210c merge → buffer → A000) and pixel-match; then port FUN_1c94/FUN_210c to TS (or RAM-extract the compose buffer per slot as fallback oracle) → pixel-parity.
