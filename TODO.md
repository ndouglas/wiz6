...
- #022 [open] — Wire SkillTrainScreen.tsx viewer screen to match parity render (parity test passes at 100% with the standalone `renderSkillTrain` helper; the live viewer screen `packages/viewer/src/pages/roster/creation/screens/SkillTrainScreen.tsx` is still a placeholder)
  - Parity recipe (in `tools/parity/screen-parity.test.ts` → `renderSkillTrain`): draft.derived = {hpInitial,stamina,level=1,xp:0}; portrait baked at top (1..3,1..3) chars 0x48..0x50 attr 0x02; age fields overwritten (row 2 " 20"@0xe0, row 3 "  1"@0xc0); `skillTrain` temp window (20×16 @ pixel (160,32), engine struct at DGROUP+0x7e26) drawn byte-exact — frame chrome (wfont1), "WEAPONRY" header @0x03 with attr-0x04 brackets, 9 skill rows attr 0x20 left-aligned + attr 0x10 values right-aligned at col 18, selection cursor 'd'@0x40 at (15,3), "SKILL POINTS  5" at row 14 (label @0x90, value @0x10). bottomBar prompts (MSG 0x0262, 0x025e + "   " + 0x025f, 0x0260) all @0x03.
  - Wiring tasks: (1) load wport1/2/3 + select wportN.portraits[portraitIdx % 14] via `state.draft.portrait`; (2) pass through to the screen as a global "post-portrait font2 patch" (currently PortraitPickerScreen does this per-render; the engine effectively makes wfont2 sticky after the picker — consider hoisting the patch into CreationPage so EVERY post-portrait screen inherits it); (3) state-driven category cycle (WEAPONRY → PHYSICAL → PERSONAL → ACADEMIA) on "PRESS ▶ FOR NEXT CATEGORY"; (4) cursor row + arrow-key bindings already in SkillTrainScreen — re-do the render using the parity-test layout as the spec.
  - Open RE items: the differing left-vert glyph 0x0f vs 0x0d on rows 9/11 (origin unknown — reproduced as-is); whether other categories have different row counts / layouts; the engine's per-class skill list selection (currently we trust `availableSkillSlots(classIdx)`).
