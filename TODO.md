...
- #060 [open] — Character-creation spell selection screens render incorrectly (slots 1, 2, 3, and slot 4 = post-spell-select save-this-character prompt)
  ...
  - **MODEL RESOLVED (2026-05-31, via DOSBox capture):** the engine picker is NOT a flat list NOR a 6-school list — it's a **SCROLLABLE SINGLE-SPELL DETAIL view**. One spell at a time: a vertical scrollbar (↑/↓, left edge of the panel), the spell NAME rendered large/centered ("ENERGY BLAST"), a row of level pips + a "no" (⊘) icon + the realm in colour ("FIRE" in red), and a "COST" label + value box below. Our port's flat `eligibleSpells` list is fundamentally wrong and must be replaced with this detail view.
  ...
  - **⚠ NEW pool-size discrepancy (found 2026-05-31 during colour capture):** the engine's level-1 creation picker shows only **3** spells — FIRE/WATER/AIR (schools 0/1/2) — for BOTH Mage and Priest (verified by driving each in DOSBox-X; scroll clamps at AIR). Our `eligibleSpells()` in `SpellPickScreen.tsx` returns ALL ~33 spells in the class's book(s). So our picker over-lists vs the engine. The engine's actual eligible filter is narrower than `spellsInBook` (and our `SPELL_TABLE` level/school decode disagreed with what showed — model predicted Mage L1 = Fire/Water/Earth/Mental, engine showed Fire/Water/Air). RE the real creation-picker eligibility filter (likely class→starter-spell-pool, not "all book spells") before claiming the spell picker is parity-correct. Affects which spells a created caster can learn.
