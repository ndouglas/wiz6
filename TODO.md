...
- #076 [open] — Port the maze 3D view (renderer LOCATED: it's in ega.drv, not wmaze.ovr)
  ...
  - **Two ways forward:** (A) **TOOLING** — extend the core patch with a *pausing* breakpoint (stop at a CS:IP, dump regs+RAM at that instruction) or a memory-write watch on the viewport, to capture the wall rasterizer mid-frame. (B) **FRAMEBUFFER-ORACLE PORT** (no rasterizer crack needed) — combine the known geometry DATA (convergence @0x42/@0x4a, seam @0x36e4/@0x3717, walltypes @0x5220) with per-slot wall textures extracted from the engine framebuffer (already captured via fb()/screenshot), build the TS renderer to pixel-match. Path B is likely the faster route to a shippable maze view.
  - **Sprite path (separate):** to validate FUN_1c94/210c live, capture during a MONSTER encounter (sprites present) → DS + descriptor table (0x18-stride) + tile-index @0x17a + LUT @0x192. Findings: `docs/re/findings/egadrv-blit-internals.json`.
