...
- #060 [open] — Character-creation spell selection screens render incorrectly (slots 1, 2, 3, and slot 4 = post-spell-select save-this-character prompt)
  ...
  - **Remaining (data wiring, smaller):** per-spell **realm/element** (FIRE/WATER/…), **COST** value, and **level pips** aren't in our schemas — they need scenario.dbs spell metadata. The layout + spell name render now; realm/cost/pips show blank/default until that data is wired (`composeSpellPanel` already accepts them — just feed real values). Also: confirm the post-pick "SAVE THIS CHARACTER?" prompt screen renders right (likely fine — it's the standard ConfirmScreen), and consider a full-frame parity fixture once the char-sheet draft for a caster is reconstructable.
