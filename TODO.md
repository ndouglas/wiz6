...
- #019 [open] — wpcmk Phase 2 — Stages A–F COMPLETE; next: layout-refinement (now measurable via parity)
  ...
  - **Stage F (parity testing infra) COMPLETE** — Plan `…-parity-testing-infra.md`. (1) `tools/parity/decode-screen.ts` decodes the engine's exact 320×200 screen OFFLINE from a `.sav` `Vga` section (RE: `docs/re/findings/dosbox-vga-save-layout.json`; VRAM @ blob 0x80000 interleaved, 2 state-bands masked); (2) Playwright installed + `packages/viewer/e2e/` (canvas `getImageData` capture; `character-menu.spec.ts` asserts framed chrome — would catch the ring-sprite bug); (3) `tools/parity/diff-image.ts` (`compareRgba`+diff PNG) + `screen-parity.ts` (ours-vs-engine); (4) `tools/parity/sprite.ts` (renderFontGlyph/PicSprite, extractCell, assertSpriteMatches + CLI). Engine-cell-vs-ours black-fill cross-check = 100%.
  - **NEXT — layout-refinement pass** (now MEASURABLE): the parity harness puts the confirm screen at **67% pixel-match vs engine** (`screen-parity.ts`). Biggest diffs: background fill (ours dark-gray attr8 vs engine black), top-window chrome in cells the engine leaves blank, bottom text framed vs on gray, and the missing **3 rows of attribute label/value cells**. Refine the window geometry (§2 under-describes the subdivision) to match the CREATE PC / CHARACTER MENU references → target ≥80-90% match; tighten the `screen-parity.test.ts` floor as it improves.
  - **Browser-verify** `/castle/character-menu` (centering, keyboard nav) — the Playwright e2e covers structural render; a human eyeball is still worthwhile. Capture a CHARACTER-MENU-initial DOSBox save to enable the route-vs-engine `parity.spec.ts` case (currently `test.skip`).
