...
- #019 [open] — wpcmk Phase 2 — Stages A–F COMPLETE; next: layout-refinement (now measurable via parity)
  ...
  - **Deferred polish (this screen)**: the left stat-panel + right header are an empty char-sheet TEMPLATE (static); rendering a SELECTED character's actual stats/portrait into them is future work (portrait pixels still placeholder). Browser-verify `/castle/character-menu`.
