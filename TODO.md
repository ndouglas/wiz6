...
- #060 [open] — Character-creation spell selection screens render incorrectly (slots 1, 2, 3, and slot 4 = post-spell-select save-this-character prompt)
  ...
  - **Remaining (render port):** rewrite `SpellPickScreen.tsx` to draw — panel frame chrome + "SPELLS" title (row 1) + scrollbar + large spell name + realm(colour) + level pips + ⊘ icon + COST (row 14); move the "SELECT A NEW SPELL…" prompt to the bottom bar (msg 703). Nav = scroll up/down through the eligible spells (detail updates per spell), pick = Enter. Add a pixel-parity test vs the fixture (+ a cell-grid diagnostic). Note: the large spell name / scrollbar / pips are non-standard glyphs — verify each against the fixture. Also capture the post-pick "save this character?" prompt screen if it's also broken.
