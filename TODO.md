...
- #060 [open] — Character-creation spell selection screens render incorrectly (slots 1, 2, 3, and slot 4 = post-spell-select save-this-character prompt)
  ...
  - **Minor residual:** non-Fire realm COLOURS (`REALM_ATTR` for WATER/AIR/EARTH/MENTAL/DIVINE) are best-guess — only FIRE=0x40 is fixture-verified; capture a non-Fire caster's spell screen to confirm. The pip bar is rendered as the engine's fixed decorative bar (its per-spell meaning, if any, is undecoded). The in-game CAST screen (separate) is where spell COST/SP would actually display — wire scenario.dbs spell cost there.
