...
- #072 [open] — ASSAY follow-ups (verify MEDIUM bits + the inventory-picker nav order)
  ...
  - (1) **Inventory-picker cursor order — CONFIRMED BACKWARDS (2026-06-01, via SWAG capture).** Our `nextInventoryCursor` has NONE *after* the items (ArrowUp from NONE → the LAST item). The engine does the OPPOSITE: in the SWAG ADD picker, a single `up` from NONE selected the TOP item (LONGSWORD, index 0). So the fix is: `up`-from-NONE → index 0 (top). **Still need the full nav model** before rewriting (`down`-from-NONE? up/down within the list? clamp/wrap?) — one data point isn't enough. Capture a couple more picker frames (up→top, then up/down within the list) to pin it, then fix `nextInventoryCursor` once. **Reused by ASSAY + SWAG (ADD/REMOVE/DROP pickers) + future USE #038 / DROP #037** — get it right once. (The committed picker fixtures pin only the NONE frame, so this nav detail isn't pixel-gated today.)
...
- #034 [open] — Port WPCVW SWAG action (RE done 2026-06-01; meatier mutating manager — sequence after SKILL/DROP groundwork)
  ...
  - **Stage 2 IN PROGRESS:** `compose-swag-bag.ts` first pass committed (WIP, ~96.6% vs fixtures). **BLOCKED on 100%** by the SWAG popup's exact cell layout — title-band glyphs + the right-edge scrollbar (wfont2 widget) need pinning; `dump-cells.py --scan` doesn't find the wpcvw popup. Composer NOT yet wired into the page or gated. **Next:** extract the popup cells → match title-band + scrollbar → re-add the parity cases (reverted to keep the gate at 100%) → wire into `CharacterViewPage` (Stage 3) → e2e (Stage 5).
