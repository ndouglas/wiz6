...
- #060 [open] — Character-creation spell selection screens render incorrectly (slots 1, 2, 3, and slot 4 = post-spell-select save-this-character prompt)
  ...
  - **TODO — implement the real model in `SpellPickScreen.tsx` (the port is still the flat-list interim):**
    1. **Data fix:** swap Alchemist/Psionic byte5-bit labels in `spell-table.ts`/`spell-schools.ts` (bit0=Alchemist, bit1=Psionic); fix `CLASS_SPELLBOOKS`/`spellsInBook` consumers + their tests. The "Alchemist has no Fire" attribution actually describes **Psionic** (mask 0x2).
    2. **Eligibility:** replace `eligibleSpells()` (all book spells) with `(byte5 & bookMask) && level === 1`, grouped by school.
    3. **Navigation:** 3×2 school-grid cursor (left/right ±3, up/down ±1, clamped); render the cursor on the char-sheet school-mana icons; per-school spell view (blank for empty schools); enter picks + decrements the per-book counter; loop until picks exhausted.
    4. **Parity:** capture per-school engine fixtures (incl. a blank-school frame) and gate the grid + cursor pixel-exact.
