...
- #078 [open] — START NEW GAME scripted entry: remaining bits (narration + gate-walk SHIPPED)
  - **SHIPPED 2026-06-05** (branch `feat/faithful-start-new-game`, spec/plan `docs/superpowers/{specs,plans}/2026-06-05-faithful-start-new-game.md`): the full entry FSM — outer-gate **narration** (3 lines, msg 10010-12, **pixel-exact** strip gate `maze-entry-narration-parity.test.ts`) → ENTER-dismiss → **3-step forced-march gate-walk** (gy 118→121, crosses the one-way gate via unconditional step) → **free control**. Session `entryMode` FSM (`narration|gate-walk|free`) in `@wiz6/parser` `entry-sequence.ts`; level-0 `scriptedEntry` config. #025 turned out NOT to be a blocker (msg.json already decodes the full range). RE: `docs/re/findings/maze-entry-{narration,sequence}.json`.
  - **Remaining (smaller, deferred):** (1) the **"HMMMM..." front-wall bump** message (msg 10020) isn't displayed yet (the `bumpMsgId` is in the config; free-mode ENTER@wall could show it); (2) **OPTIONS / camp menu** (free-mode ENTER = "PRESS RETURN FOR OPTIONS") is a no-op stub — a separate feature; (3) the **gate-open mechanic** beyond the bump (inner gate is a hard stop); (4) the exact wmaze **scripted→free mode-flip handler** branch is modeled behaviorally (FSM), not statically pinned; (5) **multi-level** scripted entries (level-0 only — start derives from `entrance`).
