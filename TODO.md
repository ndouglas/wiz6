...
- #076 [open] — Port the maze 3D view (renderer LOCATED: it's in ega.drv, not wmaze.ovr)
  ...
  - **Next (caller/U-V — the last RE piece before porting):** find the caller that copies the ega.drv tail + drives it per depth-slice + sets the arg frame (width, sub-byte x, Y-clip, source texture ptr, VRAM flag); map to convergence/seam tables (@0x42/@0x4a/@0x36e4) for per-column U/V; capture the off-screen page + the page→VRAM blit; decode one wall texture end-to-end → pixel-match → port the EGA-planar RMW rasterizer to TS. **wmaze+0x681a/+0x6aa1 is a RED HERRING (party/panel code, 0x1b0 stride) — find the caller via the blob entry's return-stack or the rep-movs copier.** Framebuffer-oracle remains a fallback.
